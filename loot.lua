...
local ITEM_TASK_ID_MONSTER_KILL = 10006
...
local function OnUpdate(dt)
    ...
    if initializeLootWindowPos ~= true then 
        ...
        lootWindow.lootTrackerOverlay:AddAnchor("TOPLEFT", "UIParent", settings.lootOverlayX or 300, settings.lootOverlayY or 300)
...
local function OnLoad()
    ...
    eluLootEventWindow = api.Interface:CreateEmptyWindow("yourPaystubLootWindow", "UIParent")
    ...
    eluLootEventWindow:RegisterEvent("STORE_SELL")
    ...
    local lootOverlayVisible = false
...
local function OnUnload()
    local settings = api.GetSettings("Elu_Tracker")
    if eluLootEventWindow then eluLootEventWindow:Show(false) end
    api.On("UPDATE", function() return end)
    eluLootEventWindow = nil
    if lootWindow and lootWindow.lootTrackerOverlay then lootWindow.lootTrackerOverlay:Show(false) end
    lootWindow = nil
    api.SaveSettings()
end

elu_loot_addon.OnLoad = OnLoad
elu_loot_addon.OnUnload = OnUnload

return elu_loot_addon
